...
@Composable
fun CategorySetScreenComposable(navController: NavHostController, db: AppRoomDatabase) {
    val categorySetScreenRepository = CategorySetScreenRepositoryImpl(db)
    val title = AppScreen.SetScreen.CategorySet.title
    val categoryContentsComposable = @Composable { item: Item<Category> ->
        val categoryItem = item.item
    ...
    SetScreenComposable(navController, categorySetScreenRepository, title, categoryContentsComposable) { item: Item<*>, viewModel: SetScreenViewModel<Category> ->
...
fun ItemScreen(
    items: List<Category>,
    viewModel: CategorySetScreenViewModel = viewModel()
) {
    val addItem = {
        ...
        Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
            LazyVerticalGrid(
                ...
                items(items) { item ->
                    ...
                    ItemCardComposable(index = index.intValue, item = category.value, checkMode = checkMode)
                }
            }
        }
